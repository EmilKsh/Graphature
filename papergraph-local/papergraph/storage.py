...
LEGACY_PROJECT_DIR = Path(__file__).resolve().parents[1] / "papergraph_project"
...
def ensure_project_dirs() -> None:
    ...
    _copy_legacy_project_if_needed()
    for directory in [DATA_DIR, EXPORTS_DIR, NOTES_DIR]:
        directory.mkdir(parents=True, exist_ok=True)


def _copy_legacy_project_if_needed() -> None:
    """Preserve existing local data when upgrading from the old folder name."""

    if PROJECT_DIR.exists() or not LEGACY_PROJECT_DIR.exists():
        return
    shutil.copytree(LEGACY_PROJECT_DIR, PROJECT_DIR)
